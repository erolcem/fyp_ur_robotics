...
cmd = './tutorial-mb-generic-tracker-rgbd-realsense --model_color model/brick/brickstack/brick.cao'
			...
			for m in range (n_x-b):
				if(m == 0):
					for n in range (n_y-a):
						file.write('load("../brick40/brick.cao", t=['+str(m*brick_x)+';'+ str(n*brick_y)+';'+str(l*-1*brick_z)+'])'+'\n')
				else:
					for n in range (n_y):
...
for k in range (n_z):
	for j in range (n_x):
		for i in range (n_y):
			...
			file = open("model/brick/brickstack/brick.0.pos","w")
