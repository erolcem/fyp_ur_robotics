...
class MoveItik(object):
    ...
    def home2storage(self):
        ...
        self.arm.set_start_state_to_current_stat        #self.object_position = self.object_target.detections
        e() 
        self.arm.set_pose_target(target_pose, self.end_effector_link)
        traj = self.arm.plan()
        self.arm.execute(traj)
    ...
    def calibrate(self):
        ...
        if self.tf_listener.canTransform(self.to_link,self.from_link,t):
            ...
            target_pose_transferred.pose.orientation.x = current_pose.pose.orientation.x  #can't tranform the rotation well that's why transform only the translation
