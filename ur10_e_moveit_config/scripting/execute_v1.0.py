...
cmd = './tutorial-mb-generic-tracker-rgbd-realsense --model_color model/brick/brickstack/brick-stack.cao'
			...
			for m in range (n_x-b):
				if(m == 0):
					for n in range (n_y-a):
						file.write('load("../brick/brick.cao", t=['+str(m*brick_x)+';'+ str(n*brick_y)+';'+str(l*-1*brick_z)+'])'+'\n')
				else:
					for n in range (n_y):
						file.write('load("../brick/brick.cao", t=['+str((m-b)*brick_x)+';'+ str((n-a)*brick_y)+';'+str(l*-1*brick_z)+'])'+'\n')
		else:
			for m in range (n_x):
				for n in range (n_y):
					file.write('load("../brick40/brick.cao", t=['+str((m-b)*brick_x)+';'+ str((n-a)*brick_y)+';'+str(l*-1*brick_z)+'])'+'\n')
	file.write('################################################'+'\n')
...
for k in range (n_z):
	for j in range (n_x):
		for i in range (n_y):
			...
			file = open("model/brick/brickstack/brick-stack.0.pos","w")
			file.write(str(float(transformed_pose[0]))+'\n'+str(float(transformed_pose[1]))+'\n'+str(float(transformed_pose[2]))+'\n'+str(orixcam_brick)+'\n'+str(oriycam_brick)+'\n'+str(orizcam_brick))
			file.close()

			file = open("model/brick/brickstack/brick-stack.cao","w")
